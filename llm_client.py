...
Endpoints disponibles (todos en 10.0.152.198):
- :8000 → gpt-oss-20b   (alias 'gpt')
- :8001 → gpt-oss-20b   (alias 'gpt-oss-b', segunda instancia)
- :8003 → gemma-4-26b-a4b  (alias 'gemma')
"""
...
def _endpoint_config(key: ModelKey) -> tuple[str, str]:
    ...
    if key == "gpt":
        return (
            os.getenv("GPT_OSS_URL", "http://10.0.152.198:8000/v1"),
            os.getenv("GPT_OSS_MODEL", "gpt-oss-20b"),
        )
    if key == "gpt-oss-b":
        return (
            os.getenv("GPT_OSS_URL_B", "http://10.0.152.198:8001/v1"),
            os.getenv("GPT_OSS_MODEL", "gpt-oss-20b"),
        )
    if key == "gemma":
        return (
            os.getenv("GEMMA_URL", "http://10.0.152.198:8003/v1"),
